...
def parse_args():
    ...
    parser.add_argument("-A", "--add_annotaion", action="store_true", help="Images per file")

    parser.add_argument("--mapping", help="verbose output")
    parser.add_argument("--gen_id", action="store_true", help="verbose output")
...
def read_image(args):
    ...
    return {"image": imageio.imwrite(imageio.RETURN_BYTES, image, format="jpg"), "path": rel_path}
...
def main():
    ...
    cache_file = os.path.join(args.output, "cache.jsonl")
    ...
    with mp.Pool(args.process) as pool:
        with MsgPackWriter(args.output) as f:
            ...
            for i, x in enumerate(pool.imap(read_image, image_loader)):
                ...
                if args.annotation and args.add_annotaion:
                    x = {**x, **whitelist[x["path"]]}

                if args.gen_id:
                    x = {**x, "id": uuid.uuid4().hex}
                f.write(x)
                ...
                if i % 1000 == 0:
                    ...
                    print(f"{i}: {1000/(end - start):.2f} image/s")
                    start = end
